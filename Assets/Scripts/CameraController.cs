...
public class CameraController : MonoBehaviour
{
    ...
    public float forceScrollSpeedY = 0.5f;  //1秒間で動かすY座標
    ...
    void Start()
    {
        ...
        //取得してきたプレイヤ＾情報に付随するPlayerControllerコンポーネントを取得する
    ...
    void Update()
    {
        ...
        //Time.deltaTime について、Timeクラスのstatic変数（だから、無前提で使えた）.deltaTime は、いちUpdateの開始～次のUpdateが開始されるまでの時間。他にも.time（ゲームスタートからの経過時間）がある。
        if (isForceScrollX)                  //強制横スクロール
        {
        ...
        transform.position = camPos;            //カメラの更新座標。this.が省略されている（ここだけではないが）。他のオブジェクトがカメラを変更させたい場合、Camera.main.を付ければ変更できる。
        ...
        {
            //強制スクロールの場合
            if (isForceScrollX == true)     //if()の中身がtrueなら実行するので、== true は省略できる
            {
                //Debug.Log("強制スクロールなら");

                //強制的にサブ背景が動く。localとつけることで、親（カメラ）の相対値をとる。
                ...
                SubBackPositionChange("right");             //背景の入れ替え（下部の自作メソッド）
            }
            else
            {
