...
public class SoundManager : MonoBehaviour
{
    ...
    public bool restartBGM;     // WorldMapを行き来する際にBGMを再生し直したほうが良いかどうかの判断
    ...
    // 自分についている2つのAudioSourxeのコンポーネントを格納する配列
    ...
    {
        //BGM再生
        // 他のシーンなどで別のSoundManagerがstatic化していたら(elseで)Destroyする（それぞれのシーンから開始したい時のため、シーンごとにSoundManagerを置いておきたい）
        if (currentSoundManager == null)
        {
            // SoundManagerオブジェクトが最初に存在するシーンにて、そのオブジェクトをstaticなオブジェクトとして扱う
            currentSoundManager = this;     // thisとはヒエラルキーに存在しているオブジェクト（についているクラス）
            DontDestroyOnLoad(gameObject);      // 次のシーンにもそのまま引き継ぐため
        }
        else
        {
            Destroy(gameObject);
    ...
    void Start()
    {
        // 2つのAudioSourceを取得して配列audiosに格納(GetComponent"s")
    ...
    public void PlayBGM(BGMType type)
    {
        switch (type)
        {
            ...
            case BGMType.GameClear:
                audios[0].PlayOneShot(bgmGameClear);        // ループしないようにPlayOneShot
                break;
