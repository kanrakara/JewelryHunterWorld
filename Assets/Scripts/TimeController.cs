...
public class TimeController : MonoBehaviour
{
    ...
    bool isTimeOver = false;     // true=タイマー停止（教科書はpublicだが、privateにしてある）
    ...
    void Update()
    {
        if (isTimeOver == false)
        {
            times += Time.deltaTime;　// 経過時間を算出する（Time.time をそのまま使っても良さそう）
    ...
    //停止フラグを立てるメソッド UIControllerからも呼び出せる。カプセル化
