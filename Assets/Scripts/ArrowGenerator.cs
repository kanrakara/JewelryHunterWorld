...
{
    GameObject[] objects;       // ItemBoxタグが付いているオブジェクトを配列で格納
    GameObject player;
    ...
    void Start()
    {
        // ItemBoxタグのオブジェクトを複数取得、FindGameObjet"s"WithTag 複数形注意
        objects = GameObject.FindGameObjectsWithTag("ItemBox");
        for (int i = 0; i < objects.Length; i++)
        {
            objects[i].GetComponent<SpriteRenderer>().sprite = itemBoxOpen;
            //objectsに入っているのはItemBoxで、それにAdvent_ItemBoxスクリプトがついているので、その中身を探せる
